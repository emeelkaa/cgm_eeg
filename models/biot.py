...
class PositionalEncoding(nn.Module):
    def __init__(self, emb_size: int, drop_p=0.1, max_len=1000):
        super().__init__()
...
class BIOTEncoder(nn.Module):
    def __init__(self, emb_size=256, heads=8, depth=4, n_channels=16, n_fft=200, hop_length=100):
        super().__init__()
...
if __name__ == "__main__":
    import time
    model = BIOTClassifier(emb_size=64, heads=4, depth=4, n_channels=16, n_fft=200, hop_length=200).to('cuda')
    ...
    '''
    model = UnsupervisedPretrain(emb_size=64, heads=4, depth=4, n_channels=16, n_fft=200, hop_length=200).to('cuda')
    out1, out2 = model(x)
